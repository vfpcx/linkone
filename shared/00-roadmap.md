...
- **后续排期（2026-09-02 用户拍板按 A→B→C→D→F 推进）**：
  ...
  - **F7-1 · 现场代建入库（F5 扩展兑现，2026-09-07 ✅，roadmap v4.3）**：后端 `InboundRegisterDto.attachments` ≤5（N2 白名单同 registerForwardByWk）+ `registerByWk` 拍照附件落列 + `TenantBatchConfigVo.photoMode` 透传（TA 拍照开关 NONE/OPTIONAL/REQUIRED）；uni WK「现场代建入库」建单页（商户→货品→到货登记（批次/效期/过期确认/货位按开关联动）→现场拍照 ≤5（REQUIRED 必拍端侧把关，POST /api/v1/files 先传后挂）→实收确认）→ `POST /tenant/inbound` 直达 PENDING_WA_CONFIRM + 72h 商户确认链；入库作业页加「现场代建」段（集中看 72h 截止/逾期自动确认/异议 TA 仲裁/冲销撤销）+ 详情登记照片预览；后端全量 **537 绿** + vue-tsc 0 错 + 双端 build DONE；详见 progress 2026-09-07 F7-1
  - **F7-2 · 盘点批次分支（F5 扩展兑现，2026-09-07 ✅，roadmap v4.4）**：盘盈按批入库（13 §5.2 注 7 / 08 §2.2 D24，P5 顺延）——`count_sheet_items` 追加 gain 批次三列（V41，仅盘盈行可带：批次号→效期必填、生产可空，效期 40205/40206）+ `registerGainBatch`（source=STOCKTAKE 批次行 initial_qty=diff + GAIN 流水回填 batch_id——FIFO 按 initial_qty 吃进，盘盈不再混「无批次在池量」）；开关护栏 submit/审批双重（50355，撞号 50362 整体回滚）；差异托盘建议值录入补移动端（盘盈 +M 占用/盘亏释放可覆盖，空=默认建议）+ 盘盈按批折叠三字段 + 详情回显批次/生效托盘 + 批次来源标签「盘盈入库」；后端全量 **543 绿**（+6：GB-01~06）+ vue-tsc 0 错 + 双端 build DONE；详见 progress 2026-09-07 F7-2
  - **F7-3 · WA 入库确认移动化（F7-1 商户侧闭环，2026-09-07 ✅，roadmap v4.5）**：uni WA 端「入库确认」页（US-WA-09 移动化，见 09 §5.3c）——待确认/全部双页签（待确认后端按 72h 倒计时升序）+ 秒级倒计时（<12h 红警示）+ 登记详情（件数/托盘/货位/批次/登记照片预览）+ 确认接受 + 异议（预设四理由 + 补充 ≤512 + 拍照附件 ≤5，提交前实时预估 在库/冲销/差额，差额>0 二次确认进平台定责）+ 结果回显（登记/冲销/差额 + YY- 仲裁单）；工作台入口角标待确认数；后端零改动（同 admin waInbound 端点）；scope：仅代建确认链，「我的申请」提交仍 admin（电脑端）；vue-tsc 0 错 + 双端 build DONE；详见 progress 2026-09-07 F7-3
  - **F7-4 · WA 我的入库申请移动化（F7-3 边界兑现，2026-09-07 ✅，roadmap v4.6）**：uni WA「我的申请」页（source=WA_SUBMIT 正向申请链）——六态横滚过滤（全部/待受理/已受理/已入库/已驳回/已撤回）+ 卡片（同批 N 单 tag / 申请件数，CONFIRMED 实登 + 差异标黄 / 批次三字段 / 驳回原因）+ 详情 sheet（申请与实登双值 + 驳回举证照片 / 登记照片预览）+ 撤回（仅 SUBMITTED，理由必填 ≤100）/ REJECTED 复制重建带参跳新建；新建页 ≤50 行多行拆单（每商品一张同批提交）+ 商品选择 sheet + 批次开关联动显隐必填 + 过期红/30 天临期黄提示 + (skuId,batchNo) 重复 50362 行级预检；api list 对象式 +submit/withdraw；工作台真实落地「入库确认」「我的申请」双 cell + 角标（顺带修复 F7-3 半成品入口缺口）；后端零改动；vue-tsc 0 错 + 双端 build DONE；详见 progress 2026-09-07 F7-4
  - **F7-5 · admin 入库拍照补全（F7-1 电脑端收口，2026-09-08 ✅，roadmap v4.7）**：admin ta/Inbound.vue 现场代建（proxy）表单接登记照片——photoMode NONE 隐藏/OPTIONAL 选填/REQUIRED 必拍端侧把关（复用 AttachmentUpload → POST /files）+ buildRegisterPayload 落 attachments + 提交复位补清货位；REQUIRED 同语义收口到正向链登记弹窗（regValid 拦截 + 必拍提示条）；后端零改动（F7-1 已支持）；admin vue-tsc 0 错 + lint 0 + build DONE；详见 progress 2026-09-08 F7-5
  - **F7-6 · E2E 基线迁 uni（admin RT 过渡态退役前置，2026-09-08 ✅，roadmap v4.8）**：admin 内 RT 最小 H5 过渡态（/rt/store UI）退役前置——Playwright 改双工程：`chromium`（admin 5173，`e2e/*.spec.ts`）+ **`uni-rt`**（RT 正式端 uni H5 5175 hash 路由，viewport 390×844，只跑 `e2e/rt-h5/`）；买家 UI 可观测旅程迁 uni（下单 rt-buy / 卖光·钳制·空店 rt-journeys / 撮合展示 rt-featured / 375 视觉 rt-visual，9 例），admin 各 spec 瘦身为纯 API 契约（sell-flow 改 apiSubmitInquiry 造单、sell-flow-2 只留 50251/50284/50286/41303、p5a FE-02 只留契约段、w5-visual 删 RT 截图测试）；rt-h5 复用 helpers 造数零复制；uni store 页补 B-RT-03 步进钳制（inc/onQtyInput 上限=库存 + step--off）；验证 chromium **122 全绿** + uni-rt **9 全绿**；环境踩坑：Memurai 僵死（Redis 写连接关闭→90001，杀进程重启）+ vite dev 被 CodeBuddy safe-delete-shim 拦 bulk 删除崩溃（清环境变量重启）；admin RT 过渡态物理删除转 Backlog（已具备删除条件，待拍板）；详见 progress 2026-09-08 F7-6
  - **挂起不排**：E/P5-B（OSS/ASR 待外部选型+云账号）、capacity 快照 job（待环境）、US-WA-01b 容量告警（暂缓）、admin RT 过渡态物理删除（Backlog，待拍板）
...
| v4.3 | 2026-09-07 | **F7-1 现场代建入库（F5 扩展兑现）**：后端 registerByWk 拍照附件 ≤5 落列（40001 超限同 registerForward）+ batch-config 透传 photoMode（TA 拍照开关三档）；uni WK 建单页 + 入库作业「现场代建」段/详情照片预览/72h 确认状态（逾期自动确认·异议 TA 仲裁·冲销撤销）；后端全量 **537 绿**（+INB-S1-02）+ vue-tsc 0 错 + 双端 build DONE；边界：photoMode 必拍端侧把关、admin proxy 接附件与 uni WA 端确认队列待后续；详见 progress 2026-09-07 F7-1 |
| v4.4 | 2026-09-07 | **F7-2 盘点批次分支（F5 扩展兑现）**：盘盈按批入库（count_sheet_items 盘盈批次三字段 V41 + Batch registerGainBatch source=STOCKTAKE/GAIN 流水回填 + FIFO 按 initial_qty 吃进）+ 开关护栏（提交/审批双重 50355，撞号 50362 整体回滚）+ uni 差异托盘建议值录入与盘盈按批 UI + 详情回显；后端全量 **543 绿**（+6：GB-01~06）+ vue-tsc 0 错 + 双端 build DONE；边界：盘亏指定扣减批次（D24 原意 B 面）在池化 FIFO 语义下不可行，随完整批次记账方案后续；详见 progress 2026-09-07 F7-2 |
| v4.5 | 2026-09-07 | **F7-3 WA 入库确认移动化（F7-1 商户侧闭环）**：uni WA「入库确认」页——待确认（72h 升序/秒级倒计时 <12h 红）/全部双页签 + 登记详情（件数/货位/批次/照片）+ 确认 + 异议（预设理由+补充+附件≤5+实时预估差额二次确认）+ YY- 结果回显；后端零改动；工作台入口角标；scope：仅代建确认链，「我的申请」仍 admin；vue-tsc 0 错 + 双端 build DONE；详见 progress 2026-09-07 F7-3 |
| v4.6 | 2026-09-07 | **F7-4 WA 我的入库申请移动化（F7-3 边界兑现）**：uni WA「我的申请」页 + 新建页（多行拆单/商品选择 sheet/批次开关联动/50362 行级预检/过期红临期黄提示）+ 撤回/复制重建；api +submit/withdraw；工作台「入库确认」「我的申请」双 cell + 角标（修复 F7-3 入口半成品）；后端零改动；vue-tsc 0 错 + 双端 build DONE；详见 progress 2026-09-07 F7-4 |
