...
### 3.5 前端视觉验收（Visual QA）★ 强制门槛

金字塔的前端单元/E2E 只验"功能正确"，验不出**视觉可见但不报错**的缺陷（对齐、错位、溢出、间距、空态、响应式断点）。这类问题体验影响大、发现成本低，**必须单列为交付门槛**，不得因"接口通/typecheck 过"就判完成。

**规则**：
1. 任何前端页面/组件交付前，用 **Playwright 截图**关键页（登录后各角色面板、列表、表单、弹窗、RT H5），由**能读图的审阅者（主会话 / 人）逐张肉眼审**——文本模型子 Agent"看不见"截图，视觉判断**不得下放给子 Agent**。
2. 逐张对照检查项：① 元素对齐（徽标/图标/文字基线）② 无溢出/截断 ③ 间距一致（走 design tokens）④ 空态/加载态/错误态有呈现 ⑤ 关键断点（≥1280 / 768 / 375）不错版。
3. 发现的视觉问题**当缺陷登记**（进 `03-defect-findings.md`），与回归测试并列为**合并门槛**：未过视觉审的前端改动不合 main。
4. 截图脚本复用 `frontend/apps/admin/e2e/` 的登录态注入（pinia-persist key `cangchu-admin-auth`）+ `seedSellChain` 造数，避免手工点。

### 3.6 前端缺陷根治原则（开发规范）★

可见 UI 缺陷**禁止只补单点**：
1. 先判定是"选错组件 / 规范缺失"还是"用错"——如裸计数误用 `el-badge`（其浮动上标 `.is-fixed` 绝对定位上浮，与菜单文字基线错位）应改用**静态胶囊组件**。
2. 抽成 `@cangchu/ui-shared` 可复用组件（如 `NavCountBadge`），并**补进 `design-system/MASTER.md` 规范**，杜绝各页各写。
3. **全仓扫同类误用**一并替换，不留第二处。
4. 组件层修复补前端单元测试（Vitest）。
