...
- **G-8.3 手机号默认脱敏（PII · 15-pii-hardening-v2 §4 阶段2）**：所有 VO/列表/导出/通知文案中的手机号**默认只回打码号**（统一 `SmsUtil.maskPhone` → `138****1234`，前端 `maskPhone` 口径对齐）；业务确需全号的场景走独立查全号端点 `/api/v1/pii/phone-reveal`（**角色 + 归属双重校验 + 审计日志只落 operator/biz/id，不落明文**）；导出文件（Excel/PDF）与通知文案**禁止出现完整手机号**，如业务必须则 last4。
- **G-8.4 手机号检索禁 LIKE（PII · 15 §4 阶段2-2）**：手机号检索一律改「完整 11 位精确（应用侧转 hmac / 明文双列 or）+ last4 尾号」口径，禁止对手机号列做 LIKE 模糊；非手机号列（如执照号）不受限。
- **G-8.5 本人数据豁免须显式（PII）**：仅"本人查询自己的数据"可回全号（如 `listMine`），必须在 service 层显式 `masked=false` 并注释原因（先例：`WholesalerApplicationServiceImpl.toVo` 双参）；禁止因"该接口面向本人"而整体关闭脱敏。
- **G-8.6 显式例外登记：WA 员工管理页展示自家员工全号（PII · W8 产品决策 D3）**：`wa/Staff.vue` 员工列表对 **WA 管理员** 展示员工手机号全号属**登记在案的显式例外**（类比企业 HR 持有员工联系方式；员工管理端点以 `requireOwnWholesalerId` 限 WA 管理员，泄露面仅"管理自家员工的人"）。约束与失效条件：
  - V29 明文收缩后，`WholesalerEmployeeVo.phone` 必须由 `users.phone_cipher` 解密供给，**禁止**裸读明文列；
  - **不放开** WE 员工查看同事手机号（若未来放开，本例外失效，须改方案 B：列表打码 + 新增 `STAFF` biz 查全号 + 权限位门控）；
  - 审计口径仍适用 G-8.3：非 `wa/Staff` 员工管理路径外，手机号默认打码。
...
[ ] 错误：防枚举 + 不泄露堆栈
[ ] PII：手机号默认打码（maskPhone 口径统一）；全号走 reveal 端点（角色+归属校验+审计）；检索禁 LIKE；导出/通知禁全号
[ ] 契约：文档 + 前后端类型已同步
```
...
| v3 | 2026-08-31 | 补 PII 防回潮规约 G-8.3/G-8.4/G-8.5（手机号默认打码、全号走 reveal+审计、检索禁 LIKE、本人豁免显式化）+ 自检卡加 PII 项（对应 15-pii-hardening-v2 §1.2-G/§4 阶段2，W7 落点） |
| v4 | 2026-09-01 | 补 G-8.6 显式例外：WA 员工管理页展示自家员工全号（W8 产品决策 D3，方案 A 保留全号 + V29 后走 cipher 解密供给 + 未来放开 WE 即失效须改方案 B） |
