 ...
 * 不直连 StockMovementMapper。暴露回放公式消费的五锚点 + created_at（§1.5 跨月扫描用）
 * + id/reversal_of_id（争议对锚点归一：RESTORE 配对冲销日，W2 据实现备注）。
 *
 * @param id           流水 id（配对解析用）
 * @param skuId        SKU
 ...
 * @param reversalOfId 反向流水指向的原流水 id（DISPUTE_RESTORE 回指配对 DISPUTE_REVERSAL；可空）
 * @param createdAt    流水落库时间（跨月锚点差额扫描：created_at ∈ 本期 ∧ bizDate < 期初）
 */
public record BillingMovementView(Long id, Long skuId, String type, int qty, LocalDateTime bizTime,
                                  int palletDelta, Long reversalOfId, LocalDateTime createdAt) {
