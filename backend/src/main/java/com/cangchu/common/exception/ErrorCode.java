...
public enum ErrorCode {
    ...
    // ==================== P6 商品规格模型（聚合 SPU + 结构化规格，5073x，归属 product 域） ====================
    /** 自建聚合 SPU 必须带规格模板（至少一个维度），笛卡尔积以其展开生成 SKU */
    SPU_SPEC_SCHEMA_REQUIRED(50730, "规格模板不能为空（至少一个规格维度）"),
    /** 规格模板非法：维度名/取值空、重复或超长 */
    SPU_SPEC_SCHEMA_INVALID(50731, "规格模板非法（维度名与取值不能为空、重复或超长）"),
    /** 规格模板超限：维度数/单维取值数/组合总数 */
    SPU_SPEC_SCHEMA_LIMIT(50732, "规格模板超出上限（维度≤5、单维取值≤20、组合≤60）"),
    /** 组合唯一性：同一聚合 SPU 下规格组合已存在（不可重复生成） */
    SPU_SPEC_COMBO_DUPLICATED(50733, "该规格组合已存在，请勿重复生成"),
    /** 提交的规格组合与 SPU 规格模板不匹配（缺维度/取值不在模板内） */
    SPU_SPEC_COMBO_INVALID(50734, "规格组合与规格模板不匹配"),
    /** 自建聚合 SPU 不存在或无权访问（跨租户/平台标品一律假装不存在，防枚举） */
    MERCHANT_SPU_NOT_FOUND(50735, "自建聚合商品不存在或无权访问"),
    /** 批量生成 SKU 请求为空（未提交任何规格组合） */
    SPU_SKU_GEN_EMPTY(50736, "请至少选择一个规格组合生成商品"),
    /** 自建聚合 SPU 的 SKU 只能经规格批量生成，不支持在 SKU 表单里单独挂接 */
    SPU_NOT_MANUALLY_LINKABLE(50737, "商户自建聚合商品不支持单独挂接，请使用规格批量生成"),
