...
9. **文件分批加载（强制，2026-08-18 用户下发）**：代码分析/架构梳理/重构任务禁止自动递归批量读取目录、禁止通配符批量加载；单批最多读 1-4 个源码文件，流程=①先列出计划读取的文件清单待确认→②读取并小结→③等指令再下一批；新增读取文件须先征询许可；分析/重构必须等所需文件全部分批加载完再执行。会话全程不自动刷新/变更 MCP 工具列表（保护云端 Prompt 缓存）。
10. **界面文案中文化（强制，2026-07-24 用户拍板）**：一切用户可见文案（页面、表格列、tag、弹窗、toast、站内信/通知正文）**禁止直接展示角色码 OPS/TA/WK/ST/WA/WE/RT 及其他英文枚举码**，一律用中文。角色中文名以 `shared/product/01-roles-and-permissions.md` §3 为准（OPS=平台运维、TA=租户管理员/店长、WK=库管员、ST=结算员、WA=批发商管理员、WE=批发商员工、RT=终端买家）。前端统一经 ui-shared 的映射函数（roleLabel/枚举 label）转换，禁止各页散落硬编码；后端拼接的用户可见文本（通知等）同样适用。代码内部标识（枚举值、DB 字段、API 参数）不受影响。
...
> Token 节约规则已上移至父项目 `../CLAUDE.md`，全局生效。
