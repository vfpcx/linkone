/**
 * P3b W5 终验收 · 视觉矩阵截图（供 Team Lead / 用户亲检，非断言型用例）
 *
 * 覆盖：
 *  1) 未登录三页 login/register/forgot-password × 两档窄屏（390×844 / 375×667）
 *  2) 各角色关键页 @1280×800，含 P3b 新页：
 *     TA returns/stocktake/batches/clearance（+dashboard 基线）、
 *     WA returns/inbound（申请视图）+ inquiry 基线、
 *     WK 出库作业（/ta/outbound WK 登录态）、OPS dashboard/arbitrations
 *
 * 产物：<repo>/.e2e-tmp/p3b-w5-visual/*.png
...
const OUT_DIR = path.resolve(process.cwd(), '../../../.e2e-tmp/p3b-w5-visual')
...
async function shot(page: Page, name: string): Promise<void> {
  ...
  await page.waitForTimeout(900) // 入场动画/倒计时首帧静置（FE-W1 教训）
  await page.screenshot({ path: path.join(OUT_DIR, `${name}.png`), fullPage: true })
}

/** 注入登录态并直达目标页（对齐 w5-visual.spec 的 enterWithAuth） */
...
test.describe('P3bW5 视觉矩阵 · 角色页 1280', () => {
  ...
  const cases: Array<{
    ...
    { name: 'ta-returns', role: 'TA', login: (s) => s.taLogin, path: '/ta/returns' },
    { name: 'ta-stocktake', role: 'TA', login: (s) => s.taLogin, path: '/ta/stocktake' },
    { name: 'ta-batches', role: 'TA', login: (s) => s.taLogin, path: '/ta/batches' },
    { name: 'ta-clearance', role: 'TA', login: (s) => s.taLogin, path: '/ta/clearance' },
    // WA 基线 + P3b 新页（inbound=入库申请视图）
    { name: 'wa-home-inquiry', role: 'WA', login: (s) => s.waLogin, path: '/wa/inquiry' },
    { name: 'wa-inbound-apply', role: 'WA', login: (s) => s.waLogin, path: '/wa/inbound' },
    { name: 'wa-returns', role: 'WA', login: (s) => s.waLogin, path: '/wa/returns' },
    // WK 工作台
